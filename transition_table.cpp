...
#ifdef BIT_VARIANCE
        new_sum_var_actual =  new_sequence.sum_of_nrzi_squares + 2 * new_sequence.sum_per_bit_rds * rds * (sign) + SIZE_OF_ENCODED_OUTPUT_IN_BITS * rds * rds;

        new_sum_var_prediction = new_sum_var_actual +new_rds * new_rds;
        ...
        for (uint32_t state8 = 0 ; state8 < NUM_OF_STATE_NTZ_COMBINATIONS; state8++)
        {
            for (uint32_t sign_bit = 0; sign_bit < NUM_SIGNS; sign_bit++)
            {
                ...
                for (uint16_t input = 0; input < NUM_INDICES; input++)
                {
                    ...
                    if (new_state < 0 || new_state >= transition.size())
                    {
                        fprintf(stderr, "new_state %u\n",new_state);
                         fprintf(stderr, "rds=%d, state8=%d, sign=%d, input=%d, new_sum_var=%d, min_rds=%d next_state=%d, 0x%04x\n",
				rds, state8, sign, input, new_st.sum_var_actual, new_st.rds, new_st.state, new_st.codeword);
...
double Codebook::calculate_pdf_with_transition_table()
{
...
void TransitionTable::calculate_independent_probabilities(bool all)
{
